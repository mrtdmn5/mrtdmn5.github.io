...
smartGlassButton.addEventListener("click", () => {
  ...
  const img1MobileSmart = document.getElementsByClassName(
    "image-1-mobile-smart"
  );
...
const visibility = document.querySelector(".visibility1");

visibility.addEventListener("click", () => {
  // GET request to get the current visibility status
  fetch("https://mom360-82d9c-default-rtdb.firebaseio.com/.json")
    ...
    .then((data) => {
      ...
      // PUT request to update the visibility status
      fetch("https://mom360-82d9c-default-rtdb.firebaseio.com/.json", {
        method: "PUT",
        headers: {
        ...
        .then((data) => {
          ...
          if (newVisibility) visibility.textContent = "Hide Tutorial Screen";
          else visibility.textContent = "Show Tutorial Screen";
        })
        .catch((error) => {
